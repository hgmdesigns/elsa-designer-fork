import { a as getRenderingRef, f as forceUpdate, h } from './index-efd13af9.js';
import { e as eventBus } from './event-bus-4cad9569.js';
import { S as SyntaxNames } from './domain-b51fc213.js';
import { e as getOrCreateProperty, s as setActivityModelProperty, p as parseJson } from './utils-2841e02a.js';
...
class MultiTextDriver {
  display(activity, property) {
    const prop = getOrCreateProperty(activity, property.name);
    return h("elsa-multi-text-property", { propertyDescriptor: property, propertyModel: prop });
  }
  update(activity, property, form) {
  }
}
...
class MultilineDriver {
  display(activity, property) {
    const prop = getOrCreateProperty(activity, property.name);
    return h("elsa-multi-line-property", { propertyDescriptor: property, propertyModel: prop });
  }
  update(activity, property, form) {
  }
}
...
class TimepickerDriver {
  display(activity, property) {
    getOrCreateProperty(activity, property.name);
...
export { EventTypes as E, LayoutDirection as L, WorkflowDesignerMode as W, pluginManager as a, propertyDisplayManager as p, state as s };
