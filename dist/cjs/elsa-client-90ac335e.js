...
const createElsaClient = function (serverUrl) {
  const config = {
    baseURL: serverUrl
  };
  ...
  return {
    ...
    workflowDefinitionsApi: {
      list: async (page, pageSize, versionOptions) => {
        const versionOptionsString = domain.getVersionOptionsString(versionOptions);
    ...
    workflowInstancesApi: {
      list: async (page, pageSize, workflowDefinitionId, workflowStatus, orderBy, searchTerm) => {
        ...
        const queryStringItems = collection.collection.map(queryString, (v, k) => `${k}=${v}`);
...
exports.createElsaClient = createElsaClient;
